...
namespace Amazon.FPS.Samples
{
    ...
    public class GetOutstandingDebtBalanceSample
    {
        ...
        /// <param name="request">GetOutstandingDebtBalanceRequest request</param>
        public static void InvokeGetOutstandingDebtBalance(AmazonFPS service, GetOutstandingDebtBalanceRequest request)
        {
            try 
            {
                ...
                Console.WriteLine("        GetOutstandingDebtBalanceResponse");
                if (response.IsSetGetOutstandingDebtBalanceResult())
                {
                    Console.WriteLine("            GetOutstandingDebtBalanceResult");
                    GetOutstandingDebtBalanceResult  getOutstandingDebtBalanceResult = response.GetOutstandingDebtBalanceResult;
                    if (getOutstandingDebtBalanceResult.IsSetOutstandingDebt())
                    {
                        Console.WriteLine("                OutstandingDebt");
                        OutstandingDebtBalance  outstandingDebt = getOutstandingDebtBalanceResult.OutstandingDebt;
                        if (outstandingDebt.IsSetOutstandingBalance())
                        {
                            Console.WriteLine("                    OutstandingBalance");
                            Amount  outstandingBalance = outstandingDebt.OutstandingBalance;
                        ...
                        if (outstandingDebt.IsSetPendingOutBalance())
                        {
                            ...
                            Amount  pendingOutBalance = outstandingDebt.PendingOutBalance;
