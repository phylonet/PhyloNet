...
namespace Amazon.FPS.Samples
{
    ...
    public class WriteOffDebtSample
    {
        ...
        /// <param name="request">WriteOffDebtRequest request</param>
        public static void InvokeWriteOffDebt(AmazonFPS service, WriteOffDebtRequest request)
        {
            try 
            {
                ...
                Console.WriteLine("        WriteOffDebtResponse");
                if (response.IsSetWriteOffDebtResult())
                {
                    Console.WriteLine("            WriteOffDebtResult");
                    WriteOffDebtResult  writeOffDebtResult = response.WriteOffDebtResult;
                    if (writeOffDebtResult.IsSetTransactionId())
                    {
                        Console.WriteLine("                TransactionId");
                        Console.WriteLine("                    {0}", writeOffDebtResult.TransactionId);
                    }
                    if (writeOffDebtResult.IsSetTransactionStatus())
                    {
                        Console.WriteLine("                TransactionStatus");
                        Console.WriteLine("                    {0}", writeOffDebtResult.TransactionStatus);
                    }
                }
