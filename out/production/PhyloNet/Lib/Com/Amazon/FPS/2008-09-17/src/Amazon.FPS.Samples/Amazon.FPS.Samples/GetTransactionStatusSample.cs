...
namespace Amazon.FPS.Samples
{
    ...
    public class GetTransactionStatusSample
    {
        ...
        /// <param name="request">GetTransactionStatusRequest request</param>
        public static void InvokeGetTransactionStatus(AmazonFPS service, GetTransactionStatusRequest request)
        {
            try 
            {
                ...
                Console.WriteLine("        GetTransactionStatusResponse");
                if (response.IsSetGetTransactionStatusResult())
                {
                    Console.WriteLine("            GetTransactionStatusResult");
                    ...
                    if (getTransactionStatusResult.IsSetTransactionId())
                    {
                        ...
                        Console.WriteLine("                    {0}", getTransactionStatusResult.TransactionId);
                    }
                    if (getTransactionStatusResult.IsSetTransactionStatus())
                    {
                        Console.WriteLine("                TransactionStatus");
                        Console.WriteLine("                    {0}", getTransactionStatusResult.TransactionStatus);
                    }
